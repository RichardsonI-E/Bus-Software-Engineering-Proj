...
public class HomeScreen extends JPanel {
    ...
    private void handleSubmit() {
        ...
        // if there is only one station in list, label as invalid
        for (int i = 0; i < routePoints.size() - 1; i++) {
            if (routePoints.get(i) == null ||
                    routePoints.get(i) == routePoints.get(i + 1)) {
                JOptionPane.showMessageDialog(null,
                        "Your route is incomplete, please try again.",
                        "Invalid Route",
        ...
        // if the start station = end station with no stops between, label as invalid
        if (routePoints.get(0) == routePoints.get(routePoints.size() - 1)
                && routePoints.size() == 2) {
            JOptionPane.showMessageDialog(null,
                    "Your route is incomplete, please try again.",
                    "Invalid Route",
